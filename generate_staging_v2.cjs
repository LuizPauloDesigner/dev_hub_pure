...
// Fix vite.config.ts specifically for sandbox (optional but helpful to avoid white screen)
if (allFiles['vite.config.ts']) {
    allFiles['vite.config.ts'].content = allFiles['vite.config.ts'].content.replace("base: './'", "base: '/'");
}
...
// Prepare StackBlitz Object
const stackblitzFiles = {};
Object.keys(allFiles).forEach(key => {
    stackblitzFiles[key] = allFiles[key].content;
});

const htmlTemplate = `<!DOCTYPE html>
<html>
<head>
    <title>Elite Dashboard Staging V2</title>
    <meta charset="UTF-8">
    <style>
        ...
        .btn { display: block; width: 100%; background: #4f46e5; color: white; border: none; padding: 1.2rem; border-radius: 1rem; font-size: 1.1rem; font-weight: 700; cursor: pointer; transition: all 0.3s; margin-bottom: 1rem; text-decoration: none; text-align: center; }
        ...
        .alert { background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.2); color: #f59e0b; padding: 1rem; border-radius: 0.75rem; font-size: 0.9rem; margin-bottom: 1.5rem; }
    </style>
</head>
<body>
    <div class="card">
        <div class="icon">🚀</div>
        <h1>Elite Dashboard Fix</h1>
        <div class="alert">A tela branca geralmente é causada pelo caminho base incorreto. Corrigi isso automaticamente nesta versão (V2).</div>
        
        <form action="https://codesandbox.io/api/v1/sandboxes/define" method="POST" target="_blank">
            <input type="hidden" name="parameters" value="${parameters}" />
            <button type="submit" class="btn">Tentar no CodeSandbox V2</button>
        </form>
        ...
        <script>
            function openStackBlitz() {
                const files = ${JSON.stringify(stackblitzFiles)};
                StackBlitzSDK.openProject({
                    title: 'Elite Dashboard Staging',
                    description: 'Full developer dashboard staging environment',
                    template: 'node',
                    files: files,
                    settings: {
                        compile: {
        ...
        <div class="footer">
            Desta vez incluí todos os arquivos da pasta 'public' e corrigi a configuração do Vite.
        </div>
    </div>
</body>
</html>`;

fs.writeFileSync('DASHBOARD_STAGING_V2.html', htmlTemplate);
console.log('Staging V2 ready: DASHBOARD_STAGING_V2.html');
