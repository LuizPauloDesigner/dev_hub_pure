...
// Fix vite.config.ts for all platforms
if (allFiles['vite.config.ts']) {
    allFiles['vite.config.ts'] = allFiles['vite.config.ts'].replace("base: './'", "base: '/'");
}

// Sandbox config
allFiles['sandbox.config.json'] = JSON.stringify({ template: "node", container: { node: "18" } });
...
<html>
<head>
    <title>Elite Dashboard Fix V3</title>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .alert { background: rgba(34, 197, 94, 0.1); border: 1px solid rgba(34, 197, 94, 0.2); color: #4ade80; padding: 1rem; border-radius: 0.75rem; font-size: 0.9rem; margin-bottom: 1.5rem; }
    </style>
</head>
<body>
    <div class="card">
        <div class="icon">✅</div>
        <h1>Elite Dashboard V3</h1>
        <div class="alert">Removi arquivos pesados (package-lock) e protegi o código contra erros de renderização no navegador.</div>
        
        <p>Selecione uma plataforma de teste abaixo:</p>
        ...
        <button onclick="openStackBlitz()" class="btn btn-sb">Abrir no StackBlitz (Mais Rápido)</button>
        ...
        <script>
            function openStackBlitz() {
                ...
                StackBlitzSDK.openProject({
                    ...
                    description: 'Optimized environment',
                    template: 'node',
                    files: files
                }, { target: '_blank' });
            }
        ...
        <div class="footer">
            V3: Payload otimizado (${Math.round(safeProjectJson.length / 1024)}KB)
        </div>
    </div>
</body>
</html>`;

fs.writeFileSync('DASHBOARD_STAGING_V3.html', htmlTemplate);
console.log('Staging V3 ready: DASHBOARD_STAGING_V3.html');
