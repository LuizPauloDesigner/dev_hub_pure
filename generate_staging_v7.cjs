...
function getFiles(dir, basePath, files = {}) {
    ...
    for (const item of items) {
        // Strict exclusions for safety and size
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            try {
                ...
                // --- MEGA FIX V7 ---
                // Override API requests because CodeSandbox Client Sandboxes don't support proxy well.
                // Replaces fetch('/api/...) with fetch('https://[tunnel]/api/...)
...
// FOR CODESANDBOX V7: USE CLIENT SANDBOX TEMPLATE (VITE) OR AUTO-SETUP
allFiles['sandbox.config.json'] = JSON.stringify({ 
    template: "vite-react-ts",
    view: "browser"
});

// Avoid weird string injection errors by loading it as a clean var
const rawTemplateHTML = [
    "<!DOCTYPE html>",
    "<html>",
    "<head>",
    "    <title>Elite Dashboard Staging V7 (Direct Fetch)</title>",
    "    <meta charset=\"UTF-8\">",
    ...
    "    <div class=\"card\">",
    "        <div class=\"icon\">🚀</div>",
    "        <h1>Elite Dashboard V7</h1>",
    "        <div class=\"alert\">",
    "            <span class=\"status-dot\"></span> <b>Problema da Tela Branca Resolvido!</b> Removi o problema de dependências ausentes injetando um Sandbox de Frontend Nativo! Ao usar a aba preview o sistema acessará o túnel cloudflare sem proxy.",
    "        </div>",
    "",
    "        <form action=\"https://codesandbox.io/api/v1/sandboxes/define\" method=\"POST\" target=\"_blank\">",
    "            <input type=\"hidden\" name=\"parameters\" id=\"csb-params\" value=\"\" />",
    "            <button type=\"submit\" class=\"btn\">Abrir no CodeSandbox (NOVO)</button>",
    "        </form>",
    "",
    "        <div class=\"footer\" id=\"footer-txt\">",
    "            V7: Fetch Rewrite via Cloudflare + Vite TS Template",
    "        </div>",
    "    </div>",
    "<script>",
    "// Inject parameters at runtime",
    "document.getElementById('csb-params').value = 'REPLACE_ME';",
    "</script>",
    "</body>",
    "</html>"
].join('\n');
...
const finalHTML = rawTemplateHTML.replace("'REPLACE_ME'", "'" + parameters + "'");

fs.writeFileSync('DASHBOARD_STAGING_V7.html', finalHTML);
console.log('Staging V7 ready: DASHBOARD_STAGING_V7.html');
