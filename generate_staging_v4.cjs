...
// --- CRITICAL FIXES FOR V4 ---

// 1. Backend Tunnel Injection
const BACKEND_TUNNEL = "https://lakes-wrote-extensive-ballot.trycloudflare.com";
const FRONTEND_TUNNEL = "https://granny-looks-speeches-cups.trycloudflare.com";

if (allFiles['vite.config.ts']) {
    // Set base path and inject tunnel proxy
    allFiles['vite.config.ts'] = allFiles['vite.config.ts']
        .replace("base: './'", "base: '/'")
        .replace("target: 'http://localhost:5000'", `target: '${BACKEND_TUNNEL}'`);

    console.log(`Injecting Backend Tunnel: ${BACKEND_TUNNEL}`);
                                }
                                value = value >> 1;
                            }
                        }
...
// 2. Add a .env for the frontend to know the tunnel if needed
allFiles['.env'] = `VITE_API_URL=${BACKEND_TUNNEL}/api\n`;

// 3. Sandbox config & Tasks
allFiles['sandbox.config.json'] = JSON.stringify({ template: "vite-react-ts" });
allFiles['.codesandbox/tasks.json'] = JSON.stringify({
    setupTasks: [{ name: "Install", command: "npm install" }],
    tasks: { dev: { name: "Start Server", command: "npm run dev", runAtStart: true, preview: { port: 8080 } } }
});
...
<body>
    <div class="card">
        ...
        <div class="alert">
            <span class="status-dot"></span> <b>Autoinicialização Ativada:</b> Configurei o CodeSandbox para instalar as dependências e iniciar o servidor automaticamente. Além disso, a conexão segura Cloudflare continua ativa.
        </div>

        <div class="tunnel-info">
            <b>API Endpoint (Tunnel):</b><br/>
            ${BACKEND_TUNNEL}
        </div>
        
        <p>Ao abrir no StackBlitz, o sistema irá instalar as dependências e conectar ao seu banco de dados local.</p>

        <button onclick="openStackBlitz()" class="btn btn-sb">Abrir no StackBlitz (Recomendado)</button>
